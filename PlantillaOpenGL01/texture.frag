...
void main(void) {
	...
	cTex02 = vec4(max(_rfill01,0.0)*cTex02.x   , max(_gfill01,0.0)*cTex02.y   , max(_bfill01,0.0)*cTex02.z   , cTex02.w);
	cTex03 = vec4(max(_rfill02,0.0)*cTex03.x   , max(_gfill02,0.0)*cTex03.y   , max(_bfill02,0.0)*cTex03.z   , cTex03.w);
	cTex04 = vec4(max(_rkeyrabbit,0.0)*cTex04.x, max(_gkeyrabbit,0.0)*cTex04.y, max(_bkeyrabbit,0.0)*cTex04.z, cTex04.w);

	//Obtenemos la imagen final.
	cFinal = max(_mix01*cTex01,0.0)*max(_mix04*cTex04,0.0) + 
	         max(_mix01*cTex01,0.0)*max(_mix02*cTex02,0.0) + 
			 max(_mix01*cTex01,0.0)*max(_mix03*cTex03,0.0);
